...
const admin = {
    ...
    // NOVA FUNÇÃO: RENDERIZA LISTA DE CLIENTES
    ...
    generateOrderCardHTML: (o) => {
        ...
        const btnPartial = isPending ? `<button onclick="admin.registerPartialOnPending('${o.id}', ${o.total})" class="btn-modern small outline" style="margin-top:5px;"><i class="fas fa-coins"></i> Entrada</button>` : '';
            ...
            instHTML = `<div style="margin-top:10px; background:white; padding:10px; border:1px solid #eee; border-radius:8px;">
                ...
                ${items.map((i, idx) => `
                    <div style="display:flex; justify-content:space-between; padding:5px 0; border-bottom:1px dashed #eee;">
                        ...
                        ${i.paid ? '<span style="color:green">PAGO</span>' : `<button onclick="admin.quickPayInst('${o.id}', ${idx}, true)" style="font-size:0.7rem;">Baixar</button>`}
                    </div>
                `).join('')}
            </div>`;
        }
